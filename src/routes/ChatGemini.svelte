...
You are given a reference answer from Perplexity. Use it to help answer the user's question, but improve or clarify as needed.`
    ...
    async function generate(msg: string) {
        loading = true;
        if (msg.trim() !== '') {
            let marked_parsed: any = marked.parse(escapeHTML(msg));
            ...
            try {
                const response = await chat.sendMessageStream({message: combined_prompt !== '' ? combined_prompt : msg.trim()});
...
<section class="msger">
  ...
  <header class="msger-header">
    <div class="msger-header-title">
      <i class="fas fa-comment-alt"></i> Chat chit và tìm kiếm thông tin, made with 🔥 by and open source
    </div>
  ...
  <div class="msger-inputarea">
    ...
    <!--
       -<input type='checkbox' bind:this={search}/>
       -->
...
.msger-inputarea input[type="checkbox"] {
  width: 20px;
  height: 20px;
  accent-color: #579ffb; /* Modern browsers */
  margin-left: 12px;
  cursor: pointer;
  border-radius: 4px;
...
.msger-inputarea input[type="checkbox"]:focus {
  outline: none;
  box-shadow: 0 0 0 2px #579ffb55;
}
...
.msger-input {
  ...
  overflow-y: auto; /* allow scrolling if it reaches max height */
