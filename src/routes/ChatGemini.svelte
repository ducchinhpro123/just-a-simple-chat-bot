<script lang="ts">
    ...
    import { PUBLIC_PERPLEXITY_API, PUBLIC_GEMINI_API } from '$env/static/public';
    import DOMPurify from 'isomorphic-dompurify';
    import 'highlight.js/styles/github-dark.css';
    import { onMount } from 'svelte';
    import { GoogleGenAI, createPartFromUri, type ContentListUnion } from '@google/genai';
    ...
    let uploaded_file_base_64: string | null = null;
...
You are given a reference answer from Perplexity. Use it to help answer the user's question, but improve or clarify as needed.`;
    ...
    async function search_engine(prompt: string): Promise<PerplexityResponse | undefined> {
        let userMessageContent: any;
        if (uploaded_file_base_64) {
            userMessageContent = [
                { type: "text", text: prompt},
                {
                    type: "image_url",
                    image_url: {
                        url: uploaded_file_base_64
                    }
                }
            ]
        } else {
    ...
    async function handle_file_upload_change(event: Event) {
        ...
        if (input.files && input.files.length > 0) {
            ...
            if (file.type.startsWith('image/')) {
                ...
                reader.onloadend = () => {
                    uploaded_file_base_64 = reader.result as string;
                };
                reader.onerror = (error) => {
                    console.error("Error reading file as base64:", error);
                    error_msg = "Failed to read image file for API.";
                    uploaded_file_base_64 = null;
                }
...
/*             } */
    ...
    async function use_perplexity_search_engine(msg: string) {
        ...
        if (search?.checked && msg) {
            ...
            let response_from_perplexity: PerplexityResponse | undefined = await search_engine(msg);
            /* console.log(response_from_perplexity); */
            if (response_from_perplexity?.choices[0].message.content) {
                context_from_perplexity =
                    context_from_perplexity.concat(response_from_perplexity?.choices[0].message.content);
                ...
                if (response_from_perplexity.citations.length > 0) {
                    response_from_perplexity.citations.map((citation, i) => {
                        context_from_perplexity = context_from_perplexity.concat(`\t${i + 1}: ${citation}\n`);
                    });
                }
            }
    ...
    async function generate(msg: string | null) {
        ...
        let marked_parsed: any = marked.parse(escapeHTML(msg ? msg : `\n${(file_upload as any).name}`));
        ...
        try {
            ...
            if (content && content.length > 0) {
                ...
                /*     /1* message: combined_prompt *1/ */
                /*     message: content ? content : combined_prompt */
                /* }); */
            } else {
                if (combined_prompt !== '') {
                    response_stream = await chat.sendMessageStream({
                ...
                } else if (combined_prompt === '' && msg && msg.trim() !== '') {
                    response_stream = await chat.sendMessageStream({
                        message: msg
                    });
                } else {
            ...
            /* const bot_index = chat_history.length - 1; */
            /* chat_history[bot_index] = { */
            /*     ...chat_history[bot_index], */
            /*     htmlContent: 'gotcha, mlem mlem...' */
            /* } */
...
<section class="msger">
  ...
  <header class="msger-header">
    <div class="msger-header-title">
        <i class="fas fa-comment-alt"></i> Chat chit và tìm kiếm thông tin, made with 🔥 by and 
        <a href="https://github.com/ducchinhpro123/just-a-simple-chat-bot.git">open source</a>
    </div>
  ...
  <div class="msger-chat" bind:this={chat_container}>
      {#each chat_history as c}
            <div class="msg { c.sender === 'bot' ? 'left-msg' : 'right-msg' }">
              <img alt={c.sender === 'bot' ? 'bot' : 'user'} src={c.sender === 'bot' ? bot_logo : user_logo } class='msg-img'/>
  ...
  <div class="msger-inputarea">
    ...
    <div class="controls-group">
        ...
        <div class="action-buttons-group">
            ...
            <button
              ...
              disabled={loading || message.trim() === '' && file_upload == null}
            >
              {#if loading}
                  { loading_text_part }
              {:else}
                ...
                <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24">
                  <path fill="currentColor" d="M4 18.5v-5.154L9.846 12L4 10.654V5.5L19.423 12z"/>
                </svg>
              {/if}
            </button>

            <label for="file-upload-input" class="file-upload-label">
              <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" class="attach-icon">
    ...
    {#if preview_image !== ''}
        <img src={preview_image} alt="Uploaded file preview" class="uploaded-file-preview-image"/>
    {/if}
...
.msger-send-btn .send-icon { /* Target SVG inside send button specifically if needed */
...
@media (max-width: 768px) {
  ...
  .action-buttons-group {
    /* On mobile, you might want send button and attach icon to take full width or stay compact */
    /* Option 1: Both full width and stacked (if preferred over side-by-side) */
    /* flex-direction: column; */
    /* align-items: stretch; */
    /* gap: 8px; */

    /* Option 2: Side-by-side but button takes most space, icon is compact */
    /* (Default flex behavior will likely achieve this if button doesn't have flex-grow) */
  }

  .msger-send-btn {
    /* If action-buttons-group is column and align-items: stretch: */
    /* width: 100%; */
    /* box-sizing: border-box; */
    /* justify-content: center; */
  ...
  .file-upload-label {
    ...
    /* width: 100%; */
    /* box-sizing: border-box; */
    /* justify-content: center; /* Center icon if label is full width */
    /* padding: 8px; */ /* Adjust padding */

    /* Adjust padding for touch targets on mobile */
    padding: 8px;
  }
}
